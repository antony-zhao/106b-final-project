...
from rnd import RND, AtariFeatureModel, RunningMeanStd, PPONetwork, train_rnd, RND_Rollout, layer_init
...
class RobosuitePolicy(nn.Module):
    def __init__(self, n_actions=7, camera_dim=64, in_channels=6, act=nn.SELU):
        super().__init__()
        self.convs = nn.Sequential(
            ResBlock(in_channels=in_channels, out_channels=32, kernel_size=7, stride=4), # (4, 84, 84) -> (32, 28, 28)
            act(),
            ...
            act(),
            ResBlock(in_channels=128, out_channels=128, kernel_size=3, stride=2), # (128, 7, 7) -> (256, 4, 4) or 4096
            act()
        )
...
class RobosuiteValue(nn.Module):
    def __init__(self, camera_dim=64, in_channels=6, act=nn.SELU):
        super().__init__()
        self.convs = nn.Sequential(
            ResBlock(in_channels=in_channels, out_channels=32, kernel_size=7, stride=4), # (4, 84, 84) -> (32, 28, 28)
            act(),
        ...
        self.output_dim = self.compute_output_dim(in_channels, camera_dim)
        self.value = MLP(self.output_dim, 1, act=nn.Tanh, hidden_dim=64)
        self.value_rnd = MLP(self.output_dim, 1, act=nn.Tanh, hidden_dim=64)
...
class RobosuiteFeatureModel(nn.Module):
    def __init__(self, camera_dim=64, in_channels=2, num_hidden=3, feature_dim=512, act=nn.SELU):
        super().__init__()
        self.convs = nn.Sequential(
            ResBlock(in_channels=in_channels, out_channels=32, kernel_size=7, stride=4), # (4, 84, 84) -> (32, 28, 28)
            act(),
        ...
        self.output_dim = self.compute_output_dim(in_channels, camera_dim)
        self.mlp = MLP(self.output_dim, feature_dim, num_hiddens=num_hidden)
...
def main(args):
    ...
    def make_env(env_name, camera_dim, seed, framestack=3, eval=False):
        def thunk():
            env = suite.make(
    ...
    'agentview_image', 
    'Milk_to_robot0_eef_pos', 'Milk_to_robot0_eef_quat', 'Milk_pos', 'Milk_quat', 
    'Bread_to_robot0_eef_pos', 'Bread_to_robot0_eef_quat', 'Bread_pos', 'Bread_quat', 
    'Cereal_to_robot0_eef_pos', 'Cereal_to_robot0_eef_quat', 'Cereal_pos', 'Cereal_quat', 
    'Can_to_robot0_eef_pos', 'Can_to_robot0_eef_quat', 'Can_pos', 'Can_quat', 
    ...
    with torch.device(device):
        ...
        policy = RobosuitePolicy(env.action_space.shape[0], camera_dim=args.camera_dim, in_channels=args.framestack * 2)
        value = RobosuiteValue(camera_dim=args.camera_dim, in_channels=args.framestack * 2)
