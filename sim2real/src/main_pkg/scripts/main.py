...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    if task == 'line':
        ...
        target_pos[2] += 0.5 #linear path moves to a Z position above AR Tag.
        print("TARGET POSITION:", target_pos)
        trajectory = LinearTrajectory(start_position=current_position, goal_position=target_pos, total_time=18)
    elif task == 'circle':
        target_pos = tag_pos[0]
        target_pos[2] += 0.5
        print("TARGET POSITION:", target_pos)
        ...
        target_pos = tag_pos[0]
        target_pos[2] += 0.5
        target_pos_next0 = np.array([target_pos[0], target_pos[1] - 0.1, target_pos[2]])
        target_pos_next1 = np.array([target_pos[0] - 0.1, target_pos[1] - 0.1, target_pos[2]])
        target_pos_next2 = np.array([target_pos[0] - 0.1, target_pos[1], target_pos[2]])
        points = [target_pos, target_pos_next0, target_pos_next1, target_pos_next2, target_pos]
        trajectory = PolygonalTrajectory(points, total_time=20)
    else:
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        ...
        Kv = 0.001 * np.array([2, 1, 2, 0.5, 0.8, 0.8, 0])
        controller = PDJointVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'torque':
        # YOUR CODE HERE
        # Kp = 10 * np.array([1, 1, 1, 1, 1, 1, 1])
        # Kv = 1 * np.array([1, 1, 1, 1, 1, 1, 1])
        Kp = 6 * np.eye(7)
        Kv = 1 * np.eye(7)
